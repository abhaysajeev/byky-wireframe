...
(function () {
  ...
  function init(sidebar) {
    ...
    var groups = nav.querySelectorAll('.menu-item-group');
    ...
    function slide(li, open) {
      var ul = li.querySelector(':scope > .menu-sub');
      if (!ul) return;
    ...
    function closeAll() {
      for (var i = 0; i < groups.length; i++) {
        ...
        var ul = groups[i].querySelector(':scope > .menu-sub');
    ...
    nav.addEventListener('mouseover', function (e) {
      var row = e.target.closest('.menu-link');
      if (!row || !nav.contains(row)) return;
      var label = row.querySelector('.menu-label');
      showTip(row, label ? label.textContent.trim() : '');
    });
    nav.addEventListener('mouseout', function (e) {
      if (e.target.closest('.menu-link')) hideTip();
    });
    nav.addEventListener('scroll', hideTip);
    window.addEventListener('resize', hideTip);

    /* keep the active item's group open on load */
    var active = nav.querySelector('.menu-sub .menu-item.active');
    if (active) {
      var group = active.closest('.menu-item.menu-item-group') || active.parentNode.parentNode;
